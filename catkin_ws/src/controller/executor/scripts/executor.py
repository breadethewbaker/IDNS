...
# SUMMERY: Is a controller for the ardrone.
# MODFICATIONS: 
# changed Keypad 1 to 1(my labtop does not have a keypad)
# Controls:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_RETURN:
                    ...
                    if navigate == False:
                        record = False
                        rospy.Subscriber("/ground_truth/state",Odometry,capture_pos_from_sim)           
                    else:       
                        print 'Cancel current navigation first'
                elif event.key == pygame.K_1:
                    if navigate == False:
                        record = False
                        # ----------- you can change the value in order to change the shape
                        r = 5
                        phi = math.pi/6
                        steps = 4
                        # ----------
                        rospy.Subscriber("/ground_truth/state",Odometry,square_trajectory,(r,phi,steps))
...
def square_trajectory(data,(r,phi,steps)): # get destination points on an inclined circular trajectory with centre at current position of drone
    global record
    global point_array
    global orient_array
    global no_of_points

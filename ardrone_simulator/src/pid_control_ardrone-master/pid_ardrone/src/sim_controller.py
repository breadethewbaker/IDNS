...
class pidcontroller:
    ...
    def start_autopilot(self,autopilot_data):
		...
		rospy.Subscriber("/ground_truth/state",Odometry,state_loc)
                ...
                destinationBroadcaster.sendTransform((goal_pose.pose.position.x,goal_pose.pose.position.y,goal_pose.pose.position.z),
			 (target_quaternion[0], target_quaternion[1], target_quaternion[2], target_quaternion[3]), rospy.Time(), "/see_destination", "nav") 
...
#                print "rot",rot
                ...
                if self.reach_pose:
#                    self.repeat_pilot = False   # Zhu Chen, when finish the circle, stop repeating the circle again
#                    print "self.repeat_pilot",self.repeat_pilot
#                    print "self.reach_pose",self.reach_pose
#                    print "self.breaker",self.breaker
                    break
        ...
        if self.repeat_pilot == True and self.reach_pose == True and self.breaker == False:
            ...
            self.repeat_pilot = False
            self.reach_pose = False
            print 'Navigation Done, going to land!'
            drone.land()
#            for i in self.arrA:
#                print i,
#            print '\narrB'
...
#            print '\ndone'
    ...
    def reach_turn(self,targ_pos,targ_orient,curr_orient):
		...
		,curr_orient[3] * curr_orient[3] + curr_orient[0] * curr_orient[0] - curr_orient[1] * curr_orient[1] - curr_orient[2] * curr_orient[2])
...
#                    print 'Reached destination pose'
    ...
    def nextpoint(self,newpos,neworient,points_left,mode):
        print "points left is",points_left
        if points_left == 1:
...
#        loop_data = AutoPilotCmd(False,newpos,neworient,points_left,mode)
        self.start_autopilot(loop_data) #Zhu Chen, here is recursive form, after execute the start_autopilot() many times, then print the last line repeartly.
        print 'going to next point'
            
def odom_loc(data):
    record = False
    if record == False:
	write_x_y = open("odom_sim_location.txt","a")
	write_z = open("height.txt","a")
        c = (data.pose.pose.position.x,data.pose.pose.position.y,data.pose.pose.position.z)
	#print "Odom Current x, y, z: ", x,y,z
        write_x_y.write(str(c[0]))
        write_x_y.write(",")
        write_x_y.write(str(c[1]))
        write_x_y.write("\n")
	write_z.write(str(z))
	write_z.write("\n")	
	write_x_y.close()
...
def state_loc(data):
    record = False
    if record == False:
	write_x_y = open("state_sim_location.txt","a")
	#print "state Current x, y, z: ", data.pose.pose.position.x,data.pose.pose.position.y,data.pose.pose.position.z

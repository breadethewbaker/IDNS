...
class MocapOdometry:
    ...
    def __init__(self, PORT):
        ...
        print "ready to accept udp"
        self.udp_socket.accept()

    def quaternionFromEuler(self, ex, ey, ez):
        q = tf.transformations.quaternion_from_euler(ex, ey, ez,'rxyz') ################################################## Zhu Chen
        quat = Quaternion(*q)
        return quat
    ...
    def parseDataToSE3(self, data):
        ...
        Angle2 = array[4]
        Angle3 = array[5]
        q = self.quaternionFromEuler(Angle1,Angle2,Angle3)
        q.x = q.x
        q.y = q.y
        q.z = q.z
        q.w = q.w

#        q = self.quaternionFromEuler(*array[3:6])
        originQuaternion = array[6:]

        print "original quaternion from VICON: ", originQuaternion
        print "quat calc with euler angle"
        print q
